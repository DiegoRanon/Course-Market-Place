// Script to fix RLS policies for course thumbnails
...
async function fixRlsPolicies() {
  try {
    console.log("Starting RLS policy fixes for course thumbnails...");
    ...
    const { data: policies, error: policiesError } = await supabase.rpc(
      "exec_sql",
      {
        sql: "SELECT tablename, policyname, permissive, roles, cmd, qual, with_check FROM pg_policies WHERE tablename = 'courses' OR (tablename = 'objects' AND policyname LIKE '%course%')",
      }
    );
    ...
    if (policiesError) {
      ...
      console.log("⚠️ No policies found for courses or course thumbnails");
    } else {
...
// Run the function
console.log("=== Course Thumbnail RLS Policy Fix Tool ===");
fixRlsPolicies()
